...
import mx.com.softwell.fragmentos.gui.Free;
...
public class FreeToPlayController {
    ...
    public void getAll(){
        API.getApi().create(FreeService.class).getAll().enqueue(new Callback<JsonObject>() {
            @Override
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                try{
                    ...
                    if (status){
                        ...
                        ((Free) MainActivity.GLOBALS.get("Free")).actualizar(juegos);
                    }else{
                        miscController.CloseWait();
                        Log.e(TAG, message);
                    }
